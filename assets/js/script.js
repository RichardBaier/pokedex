...
function youtubeQuery(searchInput) {
  ...
  fetch(youtubeSearch)
    ...
    .then(function (response) {
      for (let i = 0; i < response.items.length; i++) {
        ...
        const thumbnail = response.items[i].snippet.thumbnails.default.url;
...
function getPokemon(searchInput) {
  ...
  fetch(APIURL)
    ...
    .then(function (pokemon) {
      ...
      heightWeight.textContent = `Height: ${height.toFixed(
        1
      )}in Weight: ${weight.toFixed(2)}lbs`;

      ability.textContent = `Special Ability: ${
      ...
      if (
        historyNameArray.indexOf(searchInput) == -1 &&
        historyNameArray.length < 8
      ) {
